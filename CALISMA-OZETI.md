...
9. **Araç ile gidiş + plaka** (izin talebi): formda tek tıkla `aracSecici('f_arac', …)` → işaretlenince plaka alanı (`f_arac_plaka`) açılır, zorunlu. Kayıt alanları `i.arac`, `i.plaka`. Yardımcılar: `aracDegis`, `aracSecili`, `aracPlakaOku`, `plakaDuzenle` ("34 ABC 123" biçimi), `plakaGecerli` (il kodu 01–81), `aracPlakaHatasi`, `aracYazi`, `aracRozet` (plakalı mavi rozet). Görünür: `izinSatiri` (Taleplerim + Onay), `izinDetay` ("Araç ile" satırı), izin raporu (`vIzinde`), takvim kayıt kartları, izin listesi CSV (`arac_ile`, `plaka`). Revize: `rv_arac` + plaka; araç/plaka değişimi de revize sayılır, kıyasta "Araç ile" satırı.
